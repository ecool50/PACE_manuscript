## bc_fit_csr_resid_sensitivity.R — BC per-cell-HC fit, CSR-residual sensitivity.
##
## Sensitivity variant of the BC canonical fit (`bc_fit_joint_kernel_redesign.R`,
## `mvpql_percell_hc.rds`): same per-cell homotypic-core contamination model
## (μ = μ_bio + ρ_i·a_ig, EB-shrunk ρ_i on the RAW cross-celltype E^tech ambient
## field a_ig, homotypic-core anchors), frNN kernel, isotropic edge correction,
## data-informed τ — with a complete-spatial-randomness (CSR) residual
## sensitivity check.  Not canonical.  Supersedes the rank-1 joint-bleed framing
## (removed 2026-06-15).
##
...
##   - the RE structure stays celltype-only (NO imageID RE block).
##
...
##
## Env (BC canonical base, R_BLEED_PERCELL):
...
## ---- R_CSR_RESID (SENSITIVITY): residualise neighbour predictors vs the CSR abundance floor ----
## Pre-fit de-confounding: N_t*(i) = N_t(i) - p_t * S_i, where p_t = global fraction of type t and
## S_i = total local kernel weight (rowSums K_bio). Removes the abundance/geometry floor from the
## EXPOSURE before the model sees it, so slopes/variance are estimated on the clustering signal.
if (nzchar(Sys.getenv("R_CSR_RESID"))) {
  p_glob <- as.numeric(table(factor(df$celltype, levels = TYPES))) / nrow(df)
  names(p_glob) <- TYPES
  S_i <- rowSums(K_bio)
  for (tc in TYPES) K_bio[, tc] <- K_bio[, tc] - p_glob[tc] * S_i
  say("    R_CSR_RESID: K_bio residualised vs CSR floor (N_t - p_t*S_i); new mean range=[%.3f, %.3f]",
      min(colMeans(K_bio)), max(colMeans(K_bio)))
}
